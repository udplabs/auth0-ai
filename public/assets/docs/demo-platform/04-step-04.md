...
6. Update your `.env` file with the following content:
    ...
    ```env
    FGA_STORE_ID=<Store ID from fga.dev>
    FGA_CLIENT_ID=<Client ID from fga.dev>
    FGA_CLIENT_SECRET=<Client Secret from fga.dev>
    ```

7. Restart the application (just to be certain everything sticks.)
