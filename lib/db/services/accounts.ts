import { createAccountEmbeddings, createDocuments } from '@/lib/ai/embedding';
...
export async function createAccounts({
	...
	if (createEmbeddings) {
		const docs = await createDocuments(newAccounts, newTransactions);
		await createAccountEmbeddings(docs);
	}
