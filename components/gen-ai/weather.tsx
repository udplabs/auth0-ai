...
export interface WeatherAtLocationData {
	latitude: number;
	longitude: number;
	elevation?: number;
...
const SAMPLE: WeatherAtLocationData = {
	latitude: 37.763283,
	longitude: -122.41286,
...
function n(num = 0): number {
	return Math.ceil(num);
}

export function Weather({ data, loading = false }: WeatherProps) {
	const isMobile = useIsMobile();

	const { current, current_units, daily, hourly } = data || {};

	const { temperature_2m: hourlyTemperatures = [] } = hourly || {};

	const currentHigh = Math.max(...hourlyTemperatures.slice(0, 24));
	const currentLow = Math.min(...hourlyTemperatures.slice(0, 24));
	const currentTemperature = current?.temperature_2m || 0;
	const temperatureUnit = current_units?.temperature_2m || '°F';

	const { time: currentTime = new Date().toISOString() } = current || {};
	const { sunrise = [], sunset = [] } = daily || {};

	const isDay =
		sunrise.length > 0 && sunset.length > 0
			? isWithinInterval(new Date(currentTime), {
					start: new Date(sunrise[0]),
					end: new Date(sunset[0]),
				})
			: false;
	...
	const displayTimes = currentTimeIndex
		? hourly?.time.slice(currentTimeIndex, currentTimeIndex + hoursToShow) || []
		: [];

	const displayTemperatures = currentTimeIndex
		? hourlyTemperatures.slice(
				currentTimeIndex,
	...
	return (
		<div
			className={cx(
				'skeleton-bg flex w-full max-w-lg flex-col gap-4 rounded-2xl p-4',
				{
					'bg-blue-400': isDay,
				},
				{
